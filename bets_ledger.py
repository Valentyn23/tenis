...
def _normalize_row_schema(row: dict) -> dict:
    normalized = {k: row.get(k, "") for k in LEDGER_FIELDS}
    if not normalized.get("strategy"):
        normalized["strategy"] = "balanced"
    return normalized


def _migrate_ledger_schema(path: Path) -> None:
    if not path.exists():
        return
    ...
    for raw in data_rows:
        ...
        if len(raw) == len(LEDGER_FIELDS):
            d = dict(zip(LEDGER_FIELDS, raw))
        elif len(raw) == len(LEGACY_LEDGER_FIELDS):
            d = dict(zip(LEGACY_LEDGER_FIELDS, raw))
            d["strategy"] = "balanced"
        elif len(raw) > len(LEDGER_FIELDS):
            d = dict(zip(LEDGER_FIELDS, raw[:len(LEDGER_FIELDS)]))
        else:
            padded = list(raw) + [""] * (len(LEGACY_LEDGER_FIELDS) - len(raw))
            d = dict(zip(LEGACY_LEDGER_FIELDS, padded[:len(LEGACY_LEDGER_FIELDS)]))
            d["strategy"] = "balanced"
        migrated.append(_normalize_row_schema(d))
