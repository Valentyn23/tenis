...
from odds_theoddsapi import list_tennis_sports, list_supported_tennis_keys
...
with st.sidebar:
    ...
    odds_provider = st.selectbox("Odds provider", options=["theodds", "rapidapi_flashscore", "both"], index=["theodds", "rapidapi_flashscore", "both"].index(SETTINGS.odds_provider if SETTINGS.odds_provider in {"theodds", "rapidapi_flashscore", "both"} else "theodds"))
...
with tab_predictions:
    if pred_meta:
        ...
        if pred_meta.get("odds_source_stats"):
            st.write("Odds source stats:", pred_meta.get("odds_source_stats"))
            tbs = (pred_meta.get("odds_source_stats") or {}).get("tournaments_by_source")
            if tbs:
                st.write("Tournaments found by source:", tbs)
        if saved_bets:
