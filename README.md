...
- `ODDS_PROVIDER` (default `theodds`) — `theodds`, `rapidapi_flashscore`, or `both`
...
RapidAPI / Flashscore (optional):
- `RAPIDAPI_KEY`
- `RAPIDAPI_HOST`
- `RAPIDAPI_FLASHSCORE_URL` (default `https://flashscore-api.p.rapidapi.com/v1/events/list`)
- `RAPIDAPI_FLASHSCORE_SPORT` (default `tennis`)
...
UI includes:
- settings panel (bankroll, risk profile, max events, odds provider, calibration toggle),
...
- source attribution in results (`source`, `tournament_key`) and summary `Tournaments found by source`.
